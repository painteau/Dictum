...
/// Lance une fenêtre modale pour modifier les réglages principaux sans editer JSON.
...
impl SettingsWindow {
    fn new(cfg: Config) -> Self {
        Self { cfg, saved: false, save_error: None, dirty: false }
    }
}
...
impl eframe::App for SettingsWindow {
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        egui::CentralPanel::default().frame(frame_style).show(ctx, |ui| {
            ui.set_min_size(Vec2::new(480.0, 440.0));
            ...
            ui.horizontal(|ui| {
                ...
                let langs = ["fr", "en", "de", "es", "it", "pt", "auto"];
                for lang in &langs {
                    let sel = self.cfg.language == *lang;
                    if ui.selectable_label(sel, RichText::new(*lang).monospace()).clicked() {
            ...
            ui.horizontal(|ui| {
                ...
                ui.checkbox(&mut self.cfg.hotkey.ctrl, "Ctrl");
                ui.checkbox(&mut self.cfg.hotkey.alt, "Alt");
                ui.checkbox(&mut self.cfg.hotkey.shift, "Shift");
                let keys = ["F1","F2","F3","F4","F5","F6","F7","F8","F9","F10","F11","F12","Insert","Home","End"];
                egui::ComboBox::from_id_source("hotkey_key")
                    .selected_text(&self.cfg.hotkey.key)
                    .show_ui(ui, |ui| {
                        for k in &keys {
                            if ui.selectable_label(self.cfg.hotkey.key == *k, *k).clicked() {
            ...
            ui.label(RichText::new(format!("Score config : {}/100 ({})", score, self.cfg.score_label())).color(color).small());
            ...
            ui.horizontal(|ui| {
                ...
                ui.add_space(8.0);
                let close_btn = egui::Button::new(RichText::new("✕ Fermer").color(Color32::GRAY))
...
/// Ouvre la fenêtre paramètres. Bloquant jusqu'à fermeture.
